...
/*$.ajax({
   ...
   $.ajax({
        type: "post",
        url:  "../VideoSurveillance/getTestInfoCount",
        data:{},
        async:false,
        dataType: "json",
        success: function (data) {
        	...
        	if(data != null && data.length > 0){
        		for(var i = 0; i < data.length; i++){
        		    ...
        		    var item={
        					...
        					'实验总数': data[i].zCount,
        					'自动采集试验数量': data[i].mCount,
        					'盲样试验数量': data[i].dCount
        		        };
        		    series.push(item);
...
function getAutoCollectionCount(){	
	$.ajax({
        type: "post",
        url:  baseUrl + "/VideoSurveillance/getAutoCollectionCount",
        data:{},
        async:false,
        dataType: "json",
        success: function (data) {
...
function getBlindCount(){
	$.ajax({
        type: "post",
        url:  baseUrl + "/VideoSurveillance/getBlindCount",
        data:{},
        dataType: "json",
        success: function (data) {
        	if(data != null && data.length > 0){
        		for(var i = 0; i < data.length; i++){
        			...
        			html += "<div class='swiper-slide'>"
				           ...
				           + "<div class='pieCon0'>"
					        ...
					        + "<div class='noIconz-pie' style='line-height: 100px;'>"
...
function getAutoCollectionList(value){
	...
	$('#autoCollection').DataTable({
        ...
        "ajax" : {
            type: "post",
            url: "../VideoSurveillance/getAutoCollectionList",
            dataSrc: "data",
            data: function (d) {
                var param = {};
...
function getBlindList(){
	$('#blindInfo').DataTable({
        ...
        "ajax" : {
            type: "post",
            url: "../VideoSurveillance/getBlindList",
            dataSrc: "data",
            data: function (d) {
                var param = {};
                return param;// 自定义需要传递的参数。
