...
$(function () {
    ...
    function getList() {
        ...
        $("#cementProductionPlan").DataTable({
            ...
            "ajax": "../cementProductionPlan/getCementProductionPlan.action?i_org_Id="+i_org_Id,
            "deferRender": true,
            ...
            "columns": [{
            	...
            	"data": "operate",
                        render: function(data, type, row) {
                        	var html = '';
                        	html +='<span class="scgl_del2"><a href="javascript:void(0)" onclick="update('+row.i_id+');"><img src="../resources/images/xiu.png" width="17" height="16" alt="查看" title="查看"></a></span>';
                        	if(row.i_isCancel == 0){
    ...
    $('#str_plan_No').blur(function(){
        $.ajax({
            type: "post",
            async: false,
            url: "../cementProductionPlan/getPlanNo.action",
            dataType: "json",
    ...
    $('#i_equ_Id').focus(function(){
        ...
        $.ajax({
            type: "post",
            async: false,
            url: "../Common/getEquipmentInfo.action",
            dataType: "json",
    ...
    function saveData () {
        ...
        if (window.editFlg == false) {
            ...
            options["str_create_Date"] = CurentTime();
        	 $.ajax({
                 type: "post",
                 url: '../cementProductionPlan/addCementProductionPlan.action',
...
//                     location.reload()             	 
               /*      if(data.message.indexOf('成功')>-1){
                 		alert(data.message);
                 		window.location.href=baseUrl + "/cementProductionPlan/c_productionplan.html";
                 	}else{
                 		alert(data.message);
                 		return;
                 	}
                     */
                 	if(data.code=="success"){
        				...
        				function(){
        					window.location.href=baseUrl + "/cementProductionPlan/c_productionplan.action";
        				});
        ...
        if(window.editFlg == true) {
            ...
            $.ajax({
                type: "post",
                url: '../cementProductionPlan/updateCementProductionPlan.action',
                data: JSON.stringify(options),
                contentType: 'application/json;charset=UTF-8',
                dataType: "json",
                success: function (data) {
...
function addName(){
 ...
 //查询施工配比编号
   ...
   function getSgpb(){
       $.ajax({
           type: "post",
           url: "../cementConstructionProp/getSgpbNo.action",
  ...
  function getLcbh(){
      $.ajax({
          type: "post",
          url: "../cement_Production/getBunker_CorrespondencesByContonid.action",
...
function update(i_id) {
    ...
    $("#str_Watering_Site").attr("disabled","disabled");
    ...
    $.ajax({
        ...
        async: false,
//        contentType : 'application/json;charset=UTF-8',
        url: "../cementProductionPlan/getProductionById.action",
        data: params,
        dataType: "json",
        success: function (data) {
            console.log(data[0])
...
function del(i_id){
    ...
    $.ajax({
        ...
        async: false,
        contentType: 'application/json;charset=UTF-8',
        url: "../cementProductionPlan/delCementProductionPlan.action",
...
$("#zfbutton").on("click",function () {
    $.ajax({
        ...
        async: false,
        contentType: 'application/json;charset=UTF-8',
        url: "../cementProductionPlan/zfProductionPlan.action",
...
function getC_productionplanSelect(){
	...
	$('#cementProductionPlan').DataTable({
        ...
        "ajax" : {
            type: "post",
            url: "../cementProductionPlan/getCementProductionPlan.action",
            dataSrc: "data",
            data: function (d) {
                var param = {};
...
function FindPlan_No(){
	var Plan_No = $("#str_plan_No").val()
	$.ajax({
        type: "post",
        url: "../cementProductionPlan/getCementProductionPlanByPlan_No.action",
        dataType: "json",
