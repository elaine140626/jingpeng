	...
	$('#list').DataTable(
			{
				...
				"ajax" : baseUrl+ "/AsphProportion/getAsphalt_Prod_Proportion.action?i_org_Id="+i_org_Id,
				"deferRender" : true,
				"columns" : [ {
					"data" : "serialNumber"
				}, {
...
function getMaterialModel(str_material_Mold, tag, obj) {
	...
	$.ajax({
		type : "POST",
		url : baseUrl + "/Common/getMaterialModel.action",
...
function getUpdateGradeCode(str_material_Mold,tag,obj) {
	...
	$.ajax({
		type : "POST",
		url : baseUrl + "/Common/getGradeCodeById.action",
		data : {"str_material_Type":0,
				"i_org_Id":i_org_Id,
		        "str_material_Mold":1,
		        "i_id":i_product_Id},
...
function addOrUpdate(tag) {
	...
	if(tag == 1) {
		obj = $("#tb input");
		url = baseUrl+ "/AsphProportion/addAsphalt_Prod_Proportion.action";
		...
		obj = $("#updatetb input");
		url = baseUrl+ "/AsphProportion/updateAsphalt_Prod_Proportion.action";
	...
	$.ajax({
		...
		success : function(data) {
			...
			if(data.code=="success"){
				...
				function(){
					window.location.href=baseUrl+ "/AsphProportion/asphaltProdProportion.action";
				});
...
function del(i_id) {
	...
	function(){
		$.ajax({
			type : "POST",
			url : baseUrl + "/AsphProportion/deletAsphalt_Prod_Proportion.action",
			data : {"i_id":i_id},
			dataType : "json",
			success : function(data) {
	...
	$('#list').DataTable(
			{
				...
				"ajax" : {
		            type: "post",
		            url: baseUrl+ "/AsphProportion/getAsphalt_Prod_Proportion.action",
		            dataSrc: "data",
		            data: function (d) {
		                var param = {};
