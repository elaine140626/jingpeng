	...
	$('#list').DataTable(
			{
				...
				"ajax" : baseUrl+ "/AsphaltGrading/getAsphalt_Gradings.action?i_org_Id="+i_org_Id,
				"deferRender" : true,
				"columns" : [ {
					"data" : "serialNumber"
				}, {
...
function addOrUpdate() {
    ...
    if (result != 100 && i == 11) {
        ...
        if(addOrupdateTag == 1) {
        	$.ajax({
         		type : "POST",
         		url :baseUrl + "/AsphaltGrading/addAsphalt_Grad.action",
         		data : {"list":list,
         		...
         		success : function(data) {
        			...
        			if(data.code=="success"){
        				...
        				function(){
        					window.location.href = baseUrl + "/AsphaltGrading/asphaltGrading.action";
        				});
        	...
        	$.ajax({
         		type : "POST",
         		url :  baseUrl + "/AsphaltGrading/updateAsphalt_Grad.action",
         		data : {"list":list,
         			    "str_remarks":$("#str_remarks").val(),
         			    "str_grade_Code":$("#str_grade_Code").val(),
         			    "i_product_Id":$("#str_material_Model").val(),
...
function show(i_id) {
	...
	$.ajax({
		type : "POST",
		url : baseUrl + "/AsphaltGrading/getAsphalt_GradDetailedById.action",
		data : {"i_id":i_id},
		dataType : "json",
		success : function(data) {
...
function del(i_id) {
	...
	function(){
		$.ajax({
	 		type : "POST",
	 		url :  baseUrl + "/AsphaltGrading/deletAsphalt_Grad.action",
	 		data : {"i_id":i_id},
	 		dataType : "json",
	 		success : function(data) {
				if(data.code=="success"){
...
function search() {
	...
	$('#list').DataTable({
		...
		"ajax" : {
            type: "post",
            url: baseUrl+ "/AsphaltGrading/getAsphalt_Gradings.action",
            dataSrc: "data",
            data: function (d) {
                var param = {};
