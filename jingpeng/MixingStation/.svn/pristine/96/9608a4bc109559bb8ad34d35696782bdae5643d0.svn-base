...
$(function () {
    ...
    $("#orgid").focus(function () {
        ...
        $.ajax({
            type: "Post",
            url: "../Organization/getCementmixingstation",
            data:{"name":"dfz"},
            dataType: "json",
            success: function (data) {
                $("#orgid").empty();
...
$("#cemenstasticbtn").on("click",function () {
    	
        var table = $('#Deviation_cement').dataTable();
        table.fnDestroy();
        $("#Deviation_cement").DataTable({
            "paging": true,
            "lengthChange": false,
            "pageLength": 14,
            "searching": false,
            ...
            "ajax": "../cemAndr/Deviation_Cement?Org_ID="+org_Ids,
            "deferRender": true,
...
function goto(url) {
    ...
    var start_Time ='';
    var end_Time ='';
    window.location.href=url+"?start_Time="+start_Time+"&end_Time="+end_Time+"&org_Ids="+org_Ids;
}
