...
function getList() {
	$('#list').DataTable({
		...
		"ajax" : baseUrl + "/AsphTargetPro/getAsphTargetPro.action?i_org_Id="+i_org_Id,
		"deferRender" : true,
		"columns" : [ 
			{"data" : "serialNumber"}, 
...
function getMaterialName(){
	...
	$.ajax({
	type : "POST",
	url : baseUrl + "/AsphTargetPro/getRawMaterial.action",
	data : params,
	dataType : "json",
	success : function(data) {
	...
	$.ajax({
		...
		success : function(data) {
			...
			if(data.code=="success"){
				...
				function(){
					window.location.href=baseUrl + "/AsphTargetPro/asphTargetPro.action";
				});
