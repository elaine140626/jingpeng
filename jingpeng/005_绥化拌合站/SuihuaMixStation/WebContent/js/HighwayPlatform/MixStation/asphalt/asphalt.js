...
function getNowFormatDate(flg) {
	if(flg == 0) {
		...
		//获取30天前的日期
		var myDate = new Date();
		var date = new Date(myDate - 3 * 3600 * 24 * 1000);
	}
	...
	$.ajax({
		...
		success: function(data) {
			...
			$.each(datas, function(i, n) {
				...
				if(isMatching == 1 && n.itemName == "生产配合比") {
					//firstUrl = datas[1].itemUrl;
				}else {
...
function changeMenu(e) {
	...
	if(jumpHref == 'index.html') {
		...
		if(jumpHref == 'warning_deviation.html' || jumpHref == 'granular_composition.html' || jumpHref == 'meshSize_Info.html') {
			$('#asphalt_search').hide();
