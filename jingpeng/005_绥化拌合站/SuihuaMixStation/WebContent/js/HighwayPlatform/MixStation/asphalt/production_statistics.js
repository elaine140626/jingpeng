...
function getList() {
	...
	$.ajax({
				type : "post",
				url : "../../PlatformAsphaltStatistics/getPlatformAsphaltStatisticsChars.action",
				data : {
					...
					$.each(data, function(i, b) {
						...
						if (b.list.length > 0) {
							...
							$.each(b.list, function(w, k) {
								if(k.Org_Name.indexOf("一标") != -1){
									smallArr.push(k.collect_Date + ":" + (k.ZongChanLiang - k.Subtractive1 - k.Subtractive2 - k.Subtractive3 - k.Subtractive4 - k.Subtractive5 - k.Subtractive6));
								}else{
									smallArr.push(k.collect_Date + ":" + k.ZongChanLiang);
								}
							})
			...
			.DataTable(
					{
						...
						"ajax" : {
							type : "post",
							url : "../../PlatformAsphaltStatistics/getPlatformAsphaltStatistics.action",
							dataSrc : "data",
							data : function(d) {
								var param = {};
								param.startTime = startTime + " " + "00:"
										+ "00:" + "00";
								param.endTime = endTime + " " + "23:" + "59:"
										+ "59";
								param.org_ID = org_ID;
						...
						"columns" : [
								...
								},
								{
									...
									render : function(data, type, row, meta){
										if(row.org_Name.indexOf("一标") != -1){
											data = data - row.subtractive1 - row.subtractive2 - row.subtractive3 - row.subtractive4 - row.subtractive5 - row.subtractive6;
										}
										return data;
									}
								},
								...
								},
								{
									"data" : "totalCount",
									render : function(data, type, row, meta) {
										return "<span><a href='production_data_href.html?startTime="
												+ startTime
												+ "&endTime="
												+ endTime
												+ "&org_ID="
												+ row.org_ID
												+ "&Analysis_Result=0' target='_blank'>"
												+ data + "</a></span>";
									}
								},
								{
									...
									render : function(data, type, row, meta) {
										return "<span><a href='production_data_href.html?startTime="
												...
												+ "&org_ID="
												+ row.org_ID
												+ "&Analysis_Result=1' target='_blank'>"
												+ data + "</a></span>";
									}
								},
								{
									...
									render : function(data, type, row, meta) {
										return "<span><a href='production_data_href.html?startTime="
												...
												+ "&org_ID="
												+ row.org_ID
												+ "&Analysis_Result=2' target='_blank'>"
												+ data + "</a></span>";
									}
								},
								{
									...
									render : function(data, type, row, meta) {
										var html = '';
										if (data != 0) {
											html = percentNum(row.heGePanShu,
													row.totalCount);
										} else {
											html = "0%";
										}
	...
	$('#lqstatisticslist_info').css('text-align', 'center');
}
//echart创建折线图
function creatChart(title, Xprams, series) {
	var myChart = echarts.init(document.getElementById('statistics_echar'));
	option = {
		title : {
			text : title,
			x : 'center'
...
function detailed(data) {
			...
			.DataTable(
					{
						...
						"ajax" : {
							type : "post",
							url : "../../PlatformAsphaltStatistics/getPlatformAsphaltStatisticsDetail.action",
							dataSrc : "data",
							data : function(d) {
								var param = {};
								param.startTime = startTime + " " + "00:"
	...
	$.fn.dataTable.ext.errMode = 'throw';//当出现问题时不弹出
	$('#lqstatisticspage_info').css('text-align', 'center');
	// 弹窗显示
