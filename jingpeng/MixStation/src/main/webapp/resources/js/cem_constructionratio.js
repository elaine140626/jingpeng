...
function getPhbNumber(){
    ...
    $.ajax({
        type: "post",
        url: "../CementConstructionProp/getTheory",
        data:{"i_product_Id":$("#str_material_Mold").val() ,"i_org_Id":i_org_Id},
        dataType: "json",
        success: function (data) {
        	 var html = "<option value=''></option>";
...
/*	 if(myselect.options[indexs]!= undefined){
	...
	$.ajax({
		type : "POST",
		url : baseUrl + "/CementConstructionProp/addSgpbXx.html",
		data : {
			//组织机构id
			"i_org_Id":i_org_Id,
			//施工配比编号
...
function yz(strr){
 ...
 function getList() {
        $('#CConstruction').DataTable({
            ...
            "ajax" : "../CementConstructionProp/getCementConstructionProp.html?i_org_Id="+i_org_Id,
            "deferRender" : true,
            "columns" : [ {
                "data" : "serialNumber"
            }, {
