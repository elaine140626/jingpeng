...
$(function () {
 ...
 i_org_Id = orgid;
    ...
    $('#llConstruction').DataTable({
        ...
        "ajax" : "../CementTheory/getCement_TheoryProportions.html?i_org_Id="+i_org_Id,
        "deferRender" : true,
        "columns" : [{
            "data" : "serialNumber"
        }, {
    ...
    $('#str_prop_Code').blur(function(){
        $.ajax({
            type: "post",
            async: false,
            url: "../CementTheory/getCementProportionCode.html",
            dataType: "json",
...
//	    	var fh="";
//	    	
//	    	if(tbody.rows.item(i).cells[2].childNodes[0].selectedOptions[0].innerText==""){
//				continue;
//			}
//	    	if(i+1<rows.length){
//	    		fh=",";
//	    	}
//	    	var mid = tbody.rows.item(i).cells[2].childNodes[0].selectedOptions[0].value;
//	    	
//	    	json+='{"i_theoProp_Id":'+'"'+$("#i_id").val()+'",';
//			if(mid==""){	
