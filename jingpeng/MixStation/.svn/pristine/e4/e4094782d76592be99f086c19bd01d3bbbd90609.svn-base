...
function getList() {
	$('#list').DataTable({
		...
		"ajax" : baseUrl + "/ProductionPlan/getProductionPlan.html?i_org_Id="+i_org_Id,
		"deferRender" : true,
		"columns" : [ 
			{"data" : "serialNumber"},
			{"data" : "str_plan_No"},
...
function getEqu_Id() {
	...
	$.ajax({
		type : "POST",
		url : baseUrl + "/Common/getEquipmentInfo",
...
function getProportionCode() {
	...
	$.ajax({
		type : "POST",
		url : baseUrl + "/Common/getProportionCode.html",
		data : {"str_material_Type":0,
			"i_org_Id":i_org_Id,
...
function getGradeCode() {
	$.ajax({
		type : "POST",
		url : baseUrl + "/Common/getGradeCode.html",
...
function addOrUpdate() {
	...
	//var i_equ_Id = $("#str_plan_No").find("option:selected").text();
	...
	if(addOrupdateTag == 1) {
		url =  baseUrl + "/ProductionPlan/addProductionPlan.html";
	} else {
		$('#i_id').val(upDateid);
		url = baseUrl + "/ProductionPlan/updateProductionPlan.html";
	}
	$.ajax({
		type : "POST",
		url : url,
		...
		success : function(data) {
			...
			if(data.code=="success"){
				...
				function(){
					window.location.href=baseUrl + "/ProductionPlan/productionPlan.html";
				});
...
function show(i_id) {
	...
	getEqu_Id();
	upDateid = i_id;
	$("#str_plan_No").attr("readonly","readonly");
	$("#loginModal").modal("show");
	var e = [];
    $(".modal").on("show.bs.modal",
    function() {
	...
	$.ajax({
		type : "POST",
		url : baseUrl + "/ProductionPlan/getProductionPlanById.html",
		data : {"i_id" : i_id},
		dataType : "json",
		success : function(data) {
...
function del(i_id) {
	...
	function(){
		$.ajax({
			type : "POST",
			url : baseUrl + "/ProductionPlan/delProductionPlan.html",
			data : {"i_id":i_id},
			dataType : "json",
			success : function(data) {
...
function search() {
	...
	$('#list').DataTable({
		...
		"ajax" : {
            type: "post",
            url: baseUrl + "/ProductionPlan/getProductionPlan.html",
            dataSrc: "data",
            data: function (d) {
                var param = {};
