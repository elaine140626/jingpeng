...
(function ($) {
    if($.fn.ajaxForm == undefined) {
        $.getScript("../../js/jquery-upload-file/js/jquery.form.js");
    }
    var feature = {};
    ...
    $.fn.uploadFile = function (options) {
        ...
        var s = $.extend({
            ...
            allowedTypes: "txt,xls,xlsx,doc,docx",
        ...
        $(uploadLabel).addClass(s.uploadButtonClass).css("width",s.uploadBtnwidth);
        ...
        this.startUpload = function () {
            $("." + this.formGroup).each(function (i, items) {
                if($(this).is('form')) {
                	$(this).attr("action",s.url);//从新为form赋值
                	$(this).submit();
                }
            });
        }
        ...
        function serializeAndUploadFiles(s, obj, files) {
            for(var i = 0; i < files.length; i++) {
                if(!isFileTypeAllowed(obj, s, files[i].name)) {
                    ...
                    layer.alert("不支持该类型文件上传，仅支持" + s.allowedTypes + "类型");
                    continue;
                }
...
//                    });
                ...
                // abort
                //	s.customShowAbort.call(this,xhr,o);
//                    pd.progressDiv.show();
//                    pd.cancel.hide();
//                    pd.done.hide();
//                    if(s.showAbort) {
...
//                    } else pd.progressbar.width('1%'); //Fix for small files
                ...
                uploadProgress: function (event, position, total, percentComplete) {
                    ...
                    if(s.customProgress){
                    	s.customProgress.call(this,percentVal,fileArray[0]);//调用外部进度显示方法
                    }
