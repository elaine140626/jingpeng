/*
 * ! jQuery Upload File Plugin version: 3.1.10 @requires jQuery v1.5 or later &
 * form plugin Copyright (c) 2013 Ravishanker Kusuma http://hayageek.com/
 */
(function(b) {
	if (b.fn.ajaxForm == undefined) {
		b.getScript(basePath+"library/jquery-upload-file/js/jquery.form.js");
	}
	var a = {};
	...
	b.fn.uploadFile = function(f) {
		...
		(function l() {
			if (b.fn.ajaxForm) {
				if (p.dragDrop) {
					var s = b('<div class="' + p.dragDropContainerClass
		...
		function r(K, E, x) {
			for (var F = 0; F < x.length; F++) {
				if (!g(E, K, x[F].name)) {
					if (K.showError) {
						b("<div class='" + K.errorClass + "'><b>" + x[F].name
								+ "</b> " + K.extErrorStr + K.allowedTypes
				...
				if (K.maxFileSize != -1 && x[F].size > K.maxFileSize) {
					if (K.showError) {
						b("<div class='" + K.errorClass + "'><b>" + x[F].name
								+ "</b> " + K.sizeErrorStr + t(K.maxFileSize)
								+ "</div>").appendTo(E.errorLog)
					}
					continue
				}
						...
						if (a.fileapi && a.formdata) {
							...
							for (var I = 0; I < H.length; I++) {
								if (z.showFileCounter) {
									F += D.fileCounter + z.fileCounterStyle
											+ H[I] + "<br>"
								} else {
									F += H[I] + "<br>"
								}
		...
		function j(y, x) {
			...
			this.preview = b("<img class='ajax-file-upload-preview' />")
					.width(x.previewWidth).height(x.previewHeight)
			...
			this.progressDiv = b("<div class='ajax-file-upload-progress'>")
					.appendTo(this.statusbar).hide();
			this.progressbar = b("<div class='ajax-file-upload-bar "
					+ y.formGroup + "'></div>").appendTo(this.progressDiv);
			this.abort = b("<div class='ajax-file-upload-red "
					+ x.abortButtonClass + " " + y.formGroup + "'>"
					+ x.abortStr + "</div>").appendTo(this.statusbar).hide();
			this.cancel = b("<div class='ajax-file-upload-red "
					+ x.cancelButtonClass + " " + y.formGroup + "'>"
					+ x.cancelStr + "</div>").appendTo(this.statusbar).hide();
			this.done = b("<div class='ajax-file-upload-green'>" + x.doneStr
					+ "</div>").appendTo(this.statusbar).hide();
			this.download = b("<div class='ajax-file-upload-green'>"
					+ x.downloadStr + "</div>").appendTo(this.statusbar).hide();
			this.del = b("<div class='ajax-file-upload-red'>" + x.deletelStr
					+ "</div>").appendTo(this.statusbar).hide();
			if (x.showQueueDiv) {
