...
function getList(id){
	$.ajax({
		...
		success : function(data) {
			...
			if(info[0].sampleSize != null && info[0].sampleSize != ''){
				// 试件尺寸
				document.getElementById("sampleSize").innerHTML=info[0].sampleSize;
			}
			...
			if(list != null && list.length > 0){
				for(var i = 0; i < list.length; i++) {
		    		group += "<tr>"
							...
							+"<td rowspan='3'>"+list[i].compressiveStrength+"</td>>"
							+"<td rowspan='3' colspan='2'>"+list[i].prop_DesignStrength+"</td>"
							+"<td rowspan='3'><img src='"+list[i].ultLoadImage1 +"' width='157' height='93'></td>"
							+"<td rowspan='3'><img src='"+list[i].ultLoadImage2 +"' width='157' height='93'></td>"
							+"<td rowspan='3'><img src='"+list[i].ultLoadImage3 +"' width='157' height='93'></td>"
							+"</tr>"
							+"<tr>"
