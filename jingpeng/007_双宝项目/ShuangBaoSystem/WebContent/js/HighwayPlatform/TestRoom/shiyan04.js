...
function getList(id){
	$.ajax({
		...
		success : function(data) {
			...
			if(info[0].specType != null && info[0].specType != ''){
				// 试件类型
				document.getElementById("specType").innerHTML=info[0].specType;
			}
			...
			if(info[0].designStrength != null && info[0].designStrength != ''){
				// 设计强度
				document.getElementById("designStrength").innerHTML=info[0].designStrength;
			}
			if(info[0].avgValue != null && info[0].avgValue != ''){
				// 平均抗压强度
				document.getElementById("avgValue").innerHTML=info[0].avgValue;
			}
			var isQualifiedTest = "";
