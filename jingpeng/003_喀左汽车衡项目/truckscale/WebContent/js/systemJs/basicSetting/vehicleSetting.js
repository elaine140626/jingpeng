...
function add(){
	var index = layer.open({
		type: 2,
		title: '新增车辆信息',
		content: 'vehicleSetting_add.html',
	});
	layer.full(index);
}
...
function update(id){
	var index = layer.open({
		type : 2,
		title: '修改车辆信息',
		content: 'vehicleSetting_add.html?id='+id,
	});
	layer.full(index);
}
...
function del(id){
	$.ajax({
		type:"POST",
		url:"../../vehicleSetting/getWeighingQueryEntityInfo.action",
		data:{"id":id},
		dataType:"json",
		success:function(data){
			if(data != null && data.length > 0){
				layer.alert("该车辆已使用，不可删除!",{
					icon:2,
					title:"提示"
				});
			}else{
				layer.confirm("确认要删除吗？",
						function(index){
							...
							$.ajax({
								type:"POST",
								url:"../../vehicleSetting/deleteCarInfo.action",
