			...
			success : function(data) {
				...
				if(data != null && data.length > 0){
					...
					if(isRelation == 0){
						...
						$("#duihuan").hide();
						$("#reportRenown").html("出 库 单");
					}else{
						$("#supplierName").html(data[0].supplierName);
					}
					//材料名称
					$("#materielName").html(data[0].materielName2);
					//规格型号
					$("#materielModel").html(data[0].materielModel2);
					//皮重（t）
					$("#tareWeight").html(data[0].tareWeight);
					//毛重（t）
					$("#grossWeight").html((data[0].tareWeight+(data[0].suttle/(data[0].proportion/100))).toFixed(4));
					//净重（t）
					$("#suttle").html((data[0].suttle/(data[0].proportion/100)).toFixed(4));
					//承运人
					...
					var qrcodeInfo = "";
					if(isRelation == 0){
						qrcodeInfo = data[0].outType+"\r\n"+data[0].tareMeasureTime+"\r\n"+data[0].customerName+"\r\n"+data[0].plateNumber+"\r\n"+data[0].materielName2+"\r\n"+data[0].materielModel2+"\r\n"+(data[0].suttle/(data[0].proportion/100)).toFixed(3);
					}else{
						qrcodeInfo = data[0].outType+"\r\n"+data[0].tareMeasureTime+"\r\n"+data[0].customerName+"\r\n"+data[0].plateNumber+"\r\n"+data[0].materielName2+"\r\n"+data[0].materielModel2+"\r\n"+(data[0].suttle/(data[0].proportion/100)).toFixed(3);
					}
					//生成二维码
