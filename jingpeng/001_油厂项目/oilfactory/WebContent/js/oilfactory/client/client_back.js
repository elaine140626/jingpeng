...
function update(id){
		$.fancybox({
			'href': 'client_back_visit_edit.html?id='+id,
			'width': 733,
			'height': 380,
			'type': 'iframe',
			...
			'onClosed': function() {
				window.location.href = 'client_back_visit.html';
			}
		});
}
//列表查询
function Client_backList(){
	 $("#Client_backList").DataTable({
         "paging": true,
         "lengthChange": false,
         "pageLength": 14,
         "searching": false,
         ...
         "ajax":baseUrl + "/Client_back/getClient_back.action",
         "deferRender": true,
         "columns": [{
         	   "data": "serialnumber"
         }, {
...
function del(id){
	...
	$.ajax({
        type: "post",
        url: baseUrl+'/Client_back/delClient_back.action',
        async:false,
        data:{"id":id},
        dataType: "json",
...
function search(){
	...
	//获取主列表 刷新
	var table = $('#Client_backList').dataTable();
	table.fnDestroy();
	//获取副列表 刷新
	var table1 = $('#Client_backList1').dataTable();
	table1.fnDestroy();
	$('#Client_backList').DataTable({
	  "paging": true,
      "lengthChange": false,
      ...
      "ajax" : {
          type: "post",
          url: baseUrl + "/Client_back/findCustomerinfoByName.action",
          dataSrc: "data",
          data: function (d) {
        	var param = {};
...
function findClickByName(id,pid){
	...
	$('#Client_backList').DataTable({
      ...
      "ajax" : {
          type: "post",
          url: baseUrl + "/Client_back/findByNameAndId.action",
          dataSrc: "data",
          data: function (d) {
        	  var param = {};
        	  param.id = id;
