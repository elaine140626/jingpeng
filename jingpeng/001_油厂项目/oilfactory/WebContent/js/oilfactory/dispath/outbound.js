...
function showZtreeS(){
	...
	$.ajax({
		...
		data: {"keyword":$("#keyword").val(),"flag":0},
		dataType: "json",
...
function showProgress(row){
	swal("进度查询", $("#salesProgress"+row).attr("title"));
}
...
function update(id, outTypeMark, tareWeight, outType) {
	if (roleFlag) {
		...
		// 出库单
		$.fancybox({
			'href' : 'outbound_edit.html?id=' + id + "&flag=0&rolecode=false",
			'width' : 800,
			'height' : 550,
			'type' : 'iframe',
			'hideOnOverlayClick' : false,
...
function copy(id){
	$.fancybox({
		'href': "outbound_edit.html?id="+id+"&flag=0",
		'width': 800,
		'height': 550,
		'type': 'iframe',
		'hideOnOverlayClick': false,
