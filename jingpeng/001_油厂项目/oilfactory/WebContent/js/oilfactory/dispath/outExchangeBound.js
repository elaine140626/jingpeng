...
$(document).ready(function() {
	...
	$("#addBtn").fancybox({
		...
		'onClosed': function() {
			window.location.href = 'outExchangeBound.html';
		}
	})
...
function showZtreeS(){
	...
	$.ajax({
		...
		data: {"keyword":$("#keyword").val(),"flag":4},
		dataType: "json",
        ...
        },
        {
            ...
            render: function(data, type, row) {
            	...
            	if(roleFlag){
                	...
                	if(row.validFlag == 1){
        	  		    html +=	"<a onclick=updateValidFlag("+row.id+",'"+row.serialId+"',"+row.suttle+",0) class='zuofei'><img src='../../img/common/zuofei.png' width='20' height='20' alt='作废' title='作废'></a>";
                	}
    	  		    html +=	"<a onclick=deleteStore("+row.id+",'"+row.serialId+"',"+row.suttle+",1)><img src=\"../../img/common/del.png\" width=\"20\" height=\"20\" alt=\"删除\" title=\"删除\"></a>";
            	}else{
...
function update(id, outTypeMark, tareWeight, outType) {
	if (roleFlag) {
		if (tareWeight != null) {
			...
			// 出库单
			$.fancybox({
				'href' : 'outExchangeBound_edit.html?id=' + id + "&flag=1",
				'width' : 800,
				'height' : 550,
				'type' : 'iframe',
				'hideOnOverlayClick' : false,
		...
		// 出库单
		$.fancybox({
			'href' : 'outExchangeBound_edit.html?id=' + id + "&flag=0&rolecode=false",
			'width' : 800,
			'height' : 550,
			'type' : 'iframe',
			'hideOnOverlayClick' : false,
...
function copy(id){
	$.fancybox({
		'href': "outExchangeBound_edit.html?id="+id+"&flag=0",
		'width': 800,
		'height': 550,
		'type': 'iframe',
		'hideOnOverlayClick': false,
...
//作废
function updateValidFlag(id, serialId, suttle, flag){
	if(suttle != null){
		swal({
			title : "该出库单已经称重，不可修改！",
			type : "error",
...
// 删除
function deleteStore(id, serialId, suttle, flag){
	if(suttle != null){
		swal({
			title : "该出库单已经称重，不可修改！",
			type : "error",
