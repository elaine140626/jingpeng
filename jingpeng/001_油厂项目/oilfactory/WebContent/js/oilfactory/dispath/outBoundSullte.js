var id = "";
var transportBalances = "";

$(function() {
	// 获取上页面传过来的值
	id = getUrlParam("id");
	transportBalances = getUrlParam("transportBalances");
	if (transportBalances == 1) {
		$("#amountTr").show();	
	} else {
		$("#amountTr").hide();
	}
	if(id != null && id != ""){
		...
		$.ajax({
			type: "post",
			url: "../../outbound/getOutBoundWeight.action",
			data:{id: id},
			dataType: "json",
			async:false,
			success: function (data) {
				if (data[0] && data.length > 0) {
					...
					if (data[0].Amount) {
						$("#amount").val(data[0].Amount / 1000);
					}
					if (data[0].FactoryTime) {
						$("#factoryTime").val(data[0].FactoryTime);
					}
					if (data[0].EnterDate) {
						$("#enterDate").val(data[0].EnterDate);
					}
				}
			}
		});
...
// 提交
function addEmpty () {
	var _url = "../../outbound/updateOutBoundWeight.action"
	$.ajax({
		type: "post",
		url: _url,
		data:{
...
function getSuttle(){
	if ($("#grossWeight").val() && $("#tareWeight").val()) {
		...
		$("#amount").val(($("#grossWeight").val() * 1 - $("#tareWeight").val() * 1) < 0 ? 0 : ($("#grossWeight").val() * 1 - $("#tareWeight").val() * 1))
	}		
}