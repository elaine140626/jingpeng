		...
		success: function (data) {
			if (data && data.length > 0) {
				...
				for(var i = 0; i < data.length; i++){
					$("#orderDetailedId").append("<option value='"+data[i].Id+"'>"+data[i].OrderDetailedNumber+" / "+data[i].materielChangeInfo+" / "+data[i].Transports+"</option>");
				}
			}
		}
	});
	...
	if (orderDetailNumberList != null && orderDetailNumberList.length > 0){
		for (var i = 0 ; i < orderDetailNumberList.length ; i++) {
			if (orderDetailNumberList[i].Id == value) {
				...
				$("#materielChangeInfo").val("由 "+ orderDetailNumberList[i].MaterielInfo	+" 兑换为 "+ orderDetailNumberList[i].materielChangeInfo +" 兑换比例为" + orderDetailNumberList[i].Proportion + "%");
				// 客户自提不能调拨
	...
	$.ajax({
		...
		success: function(data) {
			if (data != null && data.length > 0) {
				...
				getDiaoOrderNumberList(data[0].ExchangeMaterielId, orderDetailId);
			}
		}
	});
}
...
//获取调拨销售订单编号
function getDiaoOrderNumberList (exchangeMaterielId, orderDetailId) {
	$.ajax({
		type: "post",
		url: "../../outbound/getDiaoOrderNumber.action",
		data: {
			materielId: exchangeMaterielId,
