...
/*function update(id, outTypeMark, tareWeight, outType) {
	...
	var url = "";
	if(outType == 0){
		url = "outbound_edit.html";
	}else if(outType == 3){
		url = 'outEmptyBound_edit.html';
	}else if(outType == 4){
		url = 'outExchangeBound_edit.html';
	}
	$.fancybox({
		'href' : url+'?id=' + id + "&flag=0&rolecode=false",
		'width' : 800,
		'height' : 500,
		'type' : 'iframe',
		...
		'onClosed' : function() {
			window.location.href = 'outSummary.html';
		}
	});
}
*/
...
function exportInfo(){
	...
	$.ajaxFileUpload({
		url : '../../ExportMeasure/export.action',
