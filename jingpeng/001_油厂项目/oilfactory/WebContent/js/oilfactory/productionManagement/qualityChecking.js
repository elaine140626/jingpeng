...
function getList(){
	...
	// 状态
	var isApplication = $("#isApplication").val();
	var table1 = $('#qualityChecking').dataTable();
	table1.fnDestroy();
	$("#qualityChecking").DataTable({		
		"paging": true,
        "lengthChange": false,
        "pageLength": 14,
        "searching": false,
       ...
       "ajax" : {
           type: "post",
           url: "../../qualityChecking/getQualityCheck.action",
           dataSrc: "data",
           data: function (d) {       	   
         	   var param = {};
         	   param.materielName = materielName;
         	   param.materielModel = materielModel;
         	   param.isApplication = isApplication;
       	...
       	[{
       	   "data": "id"
	            ...
	            render:function(data,type,row,meta) {
	            	var html = '';
	            	if(roleFlag){
	            		html += "<a onclick=update("+row.id+","+row.isApplication+")><img src=\"../../img/common/tijiao.png\" width=\"20\" height=\"20\" alt=\"提交\" title=\"提交\"></a>";
	            	}else{
	            		html += "<a onclick=update("+row.id+",-1)><img src=\"../../img/common/tijiao.png\" width=\"20\" height=\"20\" alt=\"查看\" title=\"查看\"></a>";
	            	}
	        ...
	        },
	        {
	            ...
	            "data": "applicationPerson"// 提交人
	        }, {
	            "data": "applicationDate"// 提交时间
	        }],
       ...
       "fnRowCallback" : function(nRow, aData, iDisplayIndex){
            ...
            $("td:first", nRow).html(html);//设置序号位于第一列，并顺次加一
        	 // 调整行变色
            if(aData.isApplication == 1 && aData.adjustNumber > 0){                		
            	//设置满足条件行的背景颜色                		
            	$(nRow).css("background", "#ffcc99");                		
            	//设置满足条件行的字体颜色                		
            	$(nRow).css("color", "white !important");                	
            }else if(aData.isApplication == 0){
            	//设置满足条件行的背景颜色                		
...
function update(id, isApplication){
	if(isApplication == 0){
		...
		function(){
			var param = {};
			// 提交人
			param.applicationPerson = userInfo.name
			// 是否提交质检申请
			param.isApplication = 0;
			// 更新人
			...
			$.ajax({
				...
				success: function (data) {
					if(data.code == "success"){
						...
						function(){
							window.location.href="qualityChecking.html";
						}); },200);
					}else{
						swal({
							title: "错误提示",
...
//查看生产工艺通知单
function chakan(id){
	location.href = '../quality/productionProcessNotice_edit.html?id='+id+'&flag=3';
}