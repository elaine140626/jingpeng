...
function update(id){
		$.fancybox({
			'href': 'detectionIndex_edit.html?id='+id,
			'width': 733,
			'height': 380,
			'type': 'iframe',
			...
			'onClosed': function() {
				window.location.href = 'detectionIndex.html';
			}
		});
}
function DetectionindexList(){
	 $("#DetectionindexList").DataTable({
         ...
         "ajax":baseUrl + "/Detectionindex/getDetectionindex.action",
         "deferRender": true,
         "columns": [{
         	 "data": "id"
         }, {
...
function del(id){
	...
	function(){
		$.ajax({
	        type: "post",
	        url: baseUrl+'/Detectionindex/delDetectionindex.action',
	        async:false,
	        data:{"id":id},
	        dataType: "json",
...
function search(){
	...
	//获取主列表 刷新
	var table = $('#DetectionindexList').dataTable();
	table.fnDestroy();
	$('#DetectionindexList').DataTable({
	  "paging": true,
      "lengthChange": false,
      ...
      "ajax" : {
          type: "post",
          url: baseUrl + "/Detectionindex/findDetectionindexByName.action",
          dataSrc: "data",
          data: function (d) {
        	var param = {};
              param.testingItems = testingItems;//传递供应商名称
