...
function addCachetPersonnel(){
	...
	if(cid!=""&&cid!=null){
		...
		url = baseUrl+'/CachetPersonnel/updateCachetPersonnel.action';
	}else{
	options = {
			"cachetName" : cachetName,
			"creater":creater
	};
	url = baseUrl+'/CachetPersonnel/addCachetPersonnel.action'
	}
								...
								$.ajax({
									...
									success: function (data) {
										if(data != null){
												if(data.code == "200"){
													if(continuityAdd){
														window.location.href = 'cachetPersonnel_edit.html?continuity='+continuityAdd;
													}else{
														swal({
															title : data.message,
															text : "",
...
function getCachetPersonnel(id){
	$.ajax({
        type: "post",
        url: baseUrl+'/CachetPersonnel/getCachetPersonnel.action',
        async:false,
        data:{"id":id},
        dataType: "json",
        success: function (data) {
        	var cachetPersonnel = data.data;
        	$("#cachetName").val(cachetPersonnel[0].cachetName);
        	// 封签号
			if(cachetPersonnel[0].electronicsName){
				var fileName = cachetPersonnel[0].electronicsName
...
function deleteFiles(){
	$.ajax({ 
		type : "POST",
	    url: '../../CachetPersonnel/updateElectronicsName.action',  
	    async:false,
		data : {"id": id},
		dataType : "json",                         //服务器返回的格式,选择json或者xml貌似有问题
	    success:function(fileData){
...
function isJudge(){
	//运输起点
	var cachetName = $("#cachetName").val();
	if(cachetName.length>200){
		swal("操作失败", "姓名长度过长,请重新填写", "info");
		$("#cachetName").val("");
		return;
	}
}