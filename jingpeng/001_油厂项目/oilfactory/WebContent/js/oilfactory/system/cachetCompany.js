...
function update(id){
		$.fancybox({
			'href': 'cachetCompany_edit.html?id='+id,
			'width': 733,
			'height': 380,
			'type': 'iframe',
			...
			'onClosed': function() {
				window.location.href = 'cachetCompany.html';
			}
		});
}
function CachetCompanyList(){
	 $("#CachetCompanyList").DataTable({
         "paging": true,
         "lengthChange": false,
         "pageLength": 14,
         "searching": false,
         ...
         "ajax":baseUrl + "/CachetCompany/getCachetCompany.action",
         "deferRender": true,
         "columns": [{
         	 "data": "id"
         }, {
...
function del(id){
	...
	function(){
		$.ajax({
	        type: "post",
	        url: baseUrl+'/CachetCompany/delCachetCompany.action',
	        async:false,
	        data:{"id":id},
	        dataType: "json",
...
function search(){
	...
	//获取主列表 刷新
	var table = $('#CachetCompanyList').dataTable();
	table.fnDestroy();
	$('#CachetCompanyList').DataTable({
	  "paging": true,
      "lengthChange": false,
      ...
      "ajax" : {
          type: "post",
          url: baseUrl + "/CachetCompany/findCachetCompanyByName.action",
          dataSrc: "data",
          data: function (d) {
        	var param = {};
              param.testCompany = testCompany;//传递供应商名称
