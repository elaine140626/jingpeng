...
function getList(treeIds,levels){
	...
	$("#table1").DataTable({
        ...
        "ajax" : {
            type: "post",
            url: "../../SupervisionApplication/getSupervisionApplicationList.action",
            dataSrc: "data",
            data: function (d) {
            	param.cityId = cityId
        ...
        "columns": [
        	...
        	{"data": "supervisorPlan",
        		render:function(data,type,row,meta) {
        			var html = '';
        			html += "<a onclick='downSupervisorPlan("+row.id+")' href='javascript:;' title='下载'>"+data+"</a>";
        			return html;
        		}
        	},
        	{"data": "supervisorDetailed",
        		render:function(data,type,row,meta) {
        			var html = '';
        			html += "<a onclick='downSupervisorDetailed("+row.id+")' href='javascript:;' title='下载'>"+data+"</a>";
        			return html;
        		}
        	},
        	{"data": "reply",
	...
	if (selectedNodes != null && selectedNodes.length>0){
		if (selectedNodes[0].level != 3){
			...
			var index = layer.open({
				...
				title: '添加工程监督申请',
//				area: ['850px', '620px'],
				content: 'supervisionApplication_add.html?ProjectId='+ProjectId,
			});
			layer.full(index);
		}
...
function update(id){
	var index = layer.open({
		...
		title: '修改工程监督申请',
//		area: ['850px', '620px'],
		content: 'supervisionApplication_add.html?id='+id+"&flag=1",
	});
	layer.full(index);
}
...
function check(id){
	var index = layer.open({
		...
		title: '查看工程监督申请',
//		area: ['850px', '620px'],
		content: 'supervisionApplication_add.html?id='+id+"&flag=0",
	});
	layer.full(index);
}
...
function del(id){
	layer.confirm('确认要删除吗？', 
		function(index) {
			...
			$.ajax({
				type: 'POST',
				url: '../../SupervisionApplication/deleteSupervisionApplication.action',
...
function downSupervisorPlan(spId){
	window.location.href = "../../SupervisionApplication/downSupervisorPlan.action?id=" + spId;
}

function downSupervisorDetailed(spId){
	window.location.href = "../../SupervisionApplication/downSupervisorDetailed.action?id=" + spId;
}
...
function openFileIIs(spId){
	var index = layer.open({
		type: 2,
		title: '预览图片',
		content: 'supervisionApplicationImage.html?id='+spId,
	});
	layer.full(index);
}
