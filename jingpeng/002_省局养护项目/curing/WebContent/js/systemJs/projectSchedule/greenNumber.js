...
function getList(treeIds,levels){
	...
	$("#table1").DataTable({
        ...
        "ajax" : {
            type: "post",
            url: "../../GreenNumber/getGreenNumberList.action",
            dataSrc: "data",
            data: function (d) {
            	param.cityId = cityId
...
function getSum(treeIds,levels){
	...
	$.ajax({
		type: 'POST',
		url: '../../GreenNumber/getGreenNumberSum.action',
		data:param,
		dataType: 'json',
		success: function(data){
			if (data[0] != null){
				$("#routeLengthSum").html(data[0].routeLengthSum);
				$("#plantLengthSum").html(data[0].plantLengthSum);
			} else {
				$("#routeLengthSum").html("");
				$("#plantLengthSum").html("");
			}	
	...
	if (selectedNodes != null && selectedNodes.length>0){
		if (selectedNodes[0].level != 3){
			...
			var index = layer.open({
				...
				title: '添加绿化工程',
//				area: ['850px', '620px'],
				content: 'greenNumber_add.html?ProjectId='+ProjectId,
			});
			layer.full(index);
		}
...
function update(id){
	var index = layer.open({
		...
		title: '修改绿化工程',
//		area: ['850px', '620px'],
		content: 'greenNumber_add.html?id='+id+"&flag=1",
	});
	layer.full(index);
}
...
function copy(id){
	var index = layer.open({
		type: 2,
		title: '复制绿化工程',
		content: 'greenNumber_add.html?id='+id+"&flag=2",
	});
	layer.full(index);
}
...
function check(id){
	var index = layer.open({
		...
		title: '查看绿化工程',
//		area: ['850px', '620px'],
		content: 'greenNumber_add.html?id='+id+"&flag=0",
	});
	layer.full(index);
}
...
function del(id){
	layer.confirm('确认要删除吗？', 
		function(index) {
			...
			$.ajax({
				type: 'POST',
				url: '../../GreenNumber/deleteGreenNumber.action',
