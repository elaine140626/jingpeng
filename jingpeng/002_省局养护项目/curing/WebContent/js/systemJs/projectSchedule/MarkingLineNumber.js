...
function getList(treeIds,levels){
	...
	$("#table1").DataTable({
        ...
        "ajax" : {
            type: "post",
            url: "../../MarkingLineNumber/getMarkingLineNumberList.action",
            dataSrc: "data",
            data: function (d) {
            	param.cityId = cityId
...
function getSum(treeIds,levels){
	...
	$.ajax({
		type: 'POST',
		url: '../../MarkingLineNumber/getMarkingLineNumberSum.action',
		data:param,
		dataType: 'json',
		success: function(data){
			if (data != null){
				$("#hotmeltmarkingSum").html(data.hotmeltmarkingSum);
				$("#erasemarkingSum").html(data.erasemarkingSum);
			} else {
				$("#hotmeltmarkingSum").html("");
				$("#erasemarkingSum").html("");
			}	
		}
	});			
}
	...
	if (selectedNodes != null && selectedNodes.length>0){
		if (selectedNodes[0].level != 3){
			...
			var index = layer.open({
				...
				title: '添加标线工程',
//				area: ['850px', '620px'],
				content: 'MarkingLineNumber_add.html?ProjectId='+ProjectId,
			});
			layer.full(index);
		}
...
function update(id){
	var index = layer.open({
		...
		title: '修改标线工程',
//		area: ['850px', '620px'],
		content: 'MarkingLineNumber_add.html?id='+id+"&flag=1",
	});
	layer.full(index);
}
...
function copy(id){
	var index = layer.open({
		type: 2,
		title: '复制标线工程',
		content: 'MarkingLineNumber_add.html?id='+id+"&flag=2",
	});
	layer.full(index);
}
...
function check(id){
	var index = layer.open({
		...
		title: '查看标线工程',
//		area: ['850px', '620px'],
		content: 'MarkingLineNumber_add.html?id='+id+"&flag=0",
	});
	layer.full(index);
}
...
function del(id){
	layer.confirm('确认要删除吗？', 
		function(index) {
			...
			$.ajax({
				type: 'POST',
				url: '../../MarkingLineNumber/deleteMarkingLineNumber.action',
