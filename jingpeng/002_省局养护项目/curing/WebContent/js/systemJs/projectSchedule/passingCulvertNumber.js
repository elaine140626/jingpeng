...
function getList(treeIds,levels){
	...
	$("#table1").DataTable({
        ...
        "ajax" : {
            type: "post",
            url: "../../PassingCulvertNumber/getPassingCulvertNumberList.action",
            dataSrc: "data",
            data: function (d) {
            	param.cityId = cityId
...
function getSum(treeIds,levels){	
	...
	$.ajax({
		type: 'POST',
		url: '../../PassingCulvertNumber/getPassingCulvertNumberSum.action',
		data:param,
		dataType: 'json',
		success: function(data){
			if (data[0] != null){
				$("#crossingRebarSum").html(data[0].crossingRebarSum);
				$("#crossingConcreteSum").html(data[0].crossingConcreteSum);
				$("#coatingSum").html(data[0].coatingSum);
				$("#spreadRebarSum").html(data[0].spreadRebarSum);
				$("#spreadConcreteSum").html(data[0].spreadConcreteSum);
				$("#concreteCapSum").html(data[0].concreteCapSum);
				$("#rubbleBasicsSum").html(data[0].rubbleBasicsSum);
				$("#thickFeltSum").html(data[0].thickFeltSum);
				$("#permeableGranulesSum").html(data[0].permeableGranulesSum);
			} else {
				$("#crossingRebarSum").html("");
				$("#crossingConcreteSum").html("");
				$("#coatingSum").html("");
				$("#spreadRebarSum").html("");
				$("#spreadConcreteSum").html("");
				$("#concreteCapSum").html("");
				$("#rubbleBasicsSum").html("");
				$("#thickFeltSum").html("");
				$("#permeableGranulesSum").html("");
			}	
		}
	});			
}
	...
	if (selectedNodes != null && selectedNodes.length>0){
		if (selectedNodes[0].level != 3){
			...
			var index = layer.open({
				...
				title: '添加石砌边沟过路涵',
//				area: ['850px', '620px'],
				content: 'passingCulvertNumber_add.html?ProjectId='+ProjectId,
			});
			layer.full(index);
		}
...
function update(id){
	var index = layer.open({
		...
		title: '修改石砌边沟过路涵',
//		area: ['850px', '620px'],
		content: 'passingCulvertNumber_add.html?id='+id+"&flag=1",
	});
	layer.full(index);
}
...
function copy(id){
	var index = layer.open({
		type: 2,
		title: '复制石砌边沟过路涵',
		content: 'passingCulvertNumber_add.html?id='+id+"&flag=2",
	});
	layer.full(index);
}
...
function check(id){
	var index = layer.open({
		...
		title: '查看石砌边沟过路涵',
//		area: ['850px', '620px'],
		content: 'passingCulvertNumber_add.html?id='+id+"&flag=0",
	});
	layer.full(index);
}
...
function del(id){
	layer.confirm('确认要删除吗？', 
		function(index) {
			...
			$.ajax({
				type: 'POST',
				url: '../../PassingCulvertNumber/deletePassingCulvertNumber.action',
...
/*	$.ajax({
	...
	$.ajaxFileUpload({
		url : '../../PassingCulvertNumber/export.action',
		data:param,
		secureuri : false,
		fileElementId : 'file',
