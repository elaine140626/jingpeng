...
function getList(treeIds,levels){
	...
	$("#table1").DataTable({
        ...
        "ajax" : {
            type: "post",
            url: "../../RoadbedProtectionNumber/getRoadbedProtectionNumberList.action",
            dataSrc: "data",
            data: function (d) {
            	param.cityId = cityId
...
function getSum(treeIds,levels){
	...
	$.ajax({
		type: 'POST',
		url: '../../RoadbedProtectionNumber/getRoadbedProtectionNumberSum.action',
		data: param,
		dataType: 'json',
		success: function(data){
			if (data[0] != null){
				$("#wallLengthSum").html(data[0].wallLengthSum);
				$("#raiseHeightSum").html(data[0].raiseHeightSum);
				$("#averageHighSum").html(data[0].averageHighSum);
				$("#stoneWallBodySum").html(data[0].stoneWallBodySum);
				$("#stoneBasicsSum").html(data[0].stoneBasicsSum);
				$("#plasteringSum").html(data[0].plasteringSum);
				$("#gravelCushionSum").html(data[0].gravelCushionSum);
				$("#openExcavationSum").html(data[0].openExcavationSum);
				$("#lengthSum").html(data[0].lengthSum);
				$("#blockStoneSum").html(data[0].blockStoneSum);
				$("#eightRebarSum").html(data[0].eightRebarSum);
				$("#wireSum").html(data[0].wireSum);
			} else {
				$("#wallLengthSum").html("");
				$("#raiseHeightSum").html("");
				$("#averageHighSum").html("");
				$("#stoneWallBodySum").html("");
				$("#stoneBasicsSum").html("");
				$("#plasteringSum").html("");
				$("#gravelCushionSum").html("");
				$("#openExcavationSum").html("");
				$("#lengthSum").html("");
				$("#blockStoneSum").html("");
				$("#eightRebarSum").html("");
				$("#wireSum").html("");
	...
	if (selectedNodes != null && selectedNodes.length>0){
		if (selectedNodes[0].level != 3){
			...
			var index = layer.open({
				type: 2,
				title: '添加路基防护工程',
				content: 'roadbedProtectionNumber_add.html?ProjectId='+ProjectId,
			});
			layer.full(index);
		}
...
function update(id){
	var index = layer.open({
		type: 2,
		title: '修改路基防护工程',
		content: 'roadbedProtectionNumber_add.html?id='+id+"&flag=1",
	});
	layer.full(index);
}
...
function copy(id){
	var index = layer.open({
		type: 2,
		title: '复制路基防护工程',
		content: 'roadbedProtectionNumber_add.html?id='+id+"&flag=2",
	});
	layer.full(index);
}
...
function check(id){
	var index = layer.open({
		type: 2,
		title: '查看路基防护工程',
		content: 'roadbedProtectionNumber_add.html?id='+id+"&flag=0",
	});
	layer.full(index);
}
...
function del(id){
	layer.confirm('确认要删除吗？', 
		function(index) {
			...
			$.ajax({
				type: 'POST',
				url: '../../RoadbedProtectionNumber/deleteRoadbedProtectionNumber.action',
...
function delcol() {
    var l = $("table tbody tr td").length;
    for (var y = 0; y < $("table thead tr").length; y++) {
    	for (var i = 0; i < l; i++) {
    		if($("table thead tr").eq(y).find("th").eq(i).attr('id') == "qiangchang"){
    			$("table tbody tr").eq(i).find("td").eq(i).remove();
    			$("table thead tr").eq(y).find("th").eq(i).remove();
    			switch ("qiangchang") {
				case "qiangchang":
					$("#qiangchangSum").remove();
					$("#qiangchangTap").attr('colSpan',$("#qiangchangTap").attr('colSpan') -1)
					break;
