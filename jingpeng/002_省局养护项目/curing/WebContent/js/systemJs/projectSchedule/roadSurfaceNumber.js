...
function getList(treeIds,levels){
	...
	$("#table1").DataTable({
        ...
        "ajax" : {
            type: "post",
            url: "../../RoadSurfaceNumber/getRoadSurfaceNumberList.action",
            dataSrc: "data",
            data: function (d) {
            	param.cityId = cityId
        ...
        "columns": [
        	...
        	{"data": "gravelLower"},
        	{"data": "gravelCushion"},
        	{"data": "reinforceGravelUpper"},
        	{"data": "reinforceGravelLower"},
        	{"data": "permeable"},
        	{"data": "roadMilling"},
        	{"data": "usedMilling"},
        	{"data": "shoulder"},
...
function getSum(treeIds,levels){
	...
	$.ajax({
		type: 'POST',
		url: '../../RoadSurfaceNumber/getRoadSurfaceNumberSum.action',
		data:param,
		dataType: 'json',
		success: function(data){
			if (data[0] != null){
				$("#buildLengthSum").html(data[0].buildLengthSum);
				$("#widthSum").html(data[0].widthSum);
				$("#fineGrainSum").html(data[0].fineGrainSum);
				$("#mediumGrainSum").html(data[0].mediumGrainSum);
				$("#coarseGrainSum").html(data[0].coarseGrainSum);
				...
				$("#reinforceGravelUpperSum").html(data[0].reinforceGravelUpperSum);
				$("#reinforceGravelLowerSum").html(data[0].reinforceGravelLowerSum);
				$("#permeableSum").html(data[0].permeableSum);
				$("#roadMillingSum").html(data[0].roadMillingSum);
				$("#usedMillingSum").html(data[0].usedMillingSum);
				...
				$("#reinforceGravelUpperSum").html("");
				$("#reinforceGravelLowerSum").html("");
				$("#permeableSum").html("");
				$("#roadMillingSum").html("");
				$("#usedMillingSum").html("");
				$("#shoulderSum").html("");
	...
	if (selectedNodes != null && selectedNodes.length>0){
		if (selectedNodes[0].level != 3){
			...
			var index = layer.open({
				...
				title: '添加路面工程',
//				area: ['850px', '620px'],
				content: 'roadSurfaceNumber_add.html?ProjectId='+ProjectId,
			});
			layer.full(index);
		}
...
function update(id){
	var index = layer.open({
		...
		title: '修改路面工程',
//		area: ['850px', '620px'],
		content: 'roadSurfaceNumber_add.html?id='+id+"&flag=1",
	});
	layer.full(index);
}
...
function copy(id){
	var index = layer.open({
		type: 2,
		title: '复制路面工程',
		content: 'roadSurfaceNumber_add.html?id='+id+"&flag=2",
	});
	layer.full(index);
}
...
function check(id){
	var index = layer.open({
		...
		title: '查看路面工程',
//		area: ['850px', '620px'],
		content: 'roadSurfaceNumber_add.html?id='+id+"&flag=0",
	});
	layer.full(index);
}
...
function del(id){
	layer.confirm('确认要删除吗？', 
		function(index) {
			...
			$.ajax({
				type: 'POST',
				url: '../../RoadSurfaceNumber/deleteRoadSurfaceNumber.action',
...
/*	$.ajax({
	...
	$.ajaxFileUpload({
		url : '../../RoadSurfaceNumber/export.action',
		data:param,
		secureuri : false,
		fileElementId : 'file',
