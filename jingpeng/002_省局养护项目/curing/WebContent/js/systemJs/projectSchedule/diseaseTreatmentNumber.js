...
function getList(treeIds,levels){
	...
	$("#table1").DataTable({
        ...
        "ajax" : {
            type: "post",
            url: "../../DiseaseTreatmentNumber/getDiseaseTreatmentNumberList.action",
            dataSrc: "data",
            data: function (d) {
            	param.cityId = cityId
	...
	if (selectedNodes != null && selectedNodes.length>0){
		if (selectedNodes[0].level != 3){
			...
			var index = layer.open({
				...
				title: '添加旧路面病害处治',
//				area: ['850px', '620px'],
				content: 'diseaseTreatmentNumber_add.html?ProjectId='+ProjectId,
			});
			layer.full(index);
		}
...
function getSum(treeIds,levels){
	...
	$.ajax({
		type: 'POST',
		url: '../../DiseaseTreatmentNumber/getDiseaseTreatmentNumberSum.action',
		data:param,
		dataType: 'json',
		success: function(data){
			if (data[0] != null){
				$("#diseaseAmountSum").html(data[0].diseaseAmountSum);
				$("#caulkingSum").html(data[0].caulkingSum);
				$("#sBSConcreteSum").html(data[0].sBSConcreteSum);
				$("#gravelUpperSum").html(data[0].gravelUpperSum);
				$("#sevenConcreteSum").html(data[0].sevenConcreteSum);
				$("#permeableLayerSum").html(data[0].permeableLayerSum);
				$("#stickyLayerSum").html(data[0].stickyLayerSum);
				$("#singleAsphaltSum").html(data[0].singleAsphaltSum);
				$("#millingFourSum").html(data[0].millingFourSum);
				$("#millingTwentySum").html(data[0].millingTwentySum);
				$("#sprayAsphaltSum").html(data[0].sprayAsphaltSum);
				$("#gravelSum").html(data[0].gravelSum);
				$("#stoneChipSum").html(data[0].stoneChipSum);
				$("#digOutSum").html(data[0].digOutSum);
			} else {
				$("#diseaseAmountSum").html("");
				$("#caulkingSum").html("");
				$("#sBSConcreteSum").html("");
				$("#gravelUpperSum").html("");
				$("#sevenConcreteSum").html("");
				$("#permeableLayerSum").html("");
				$("#stickyLayerSum").html("");
				$("#singleAsphaltSum").html("");
				$("#millingFourSum").html("");
				$("#millingTwentySum").html("");
				$("#sprayAsphaltSum").html("");
				$("#gravelSum").html("");
				$("#stoneChipSum").html("");
				$("#digOutSum").html("");
			}	
...
function update(id){
	var index = layer.open({
		...
		title: '修改旧路面病害处治',
//		area: ['850px', '620px'],
		content: 'diseaseTreatmentNumber_add.html?id='+id+"&flag=1",
	});
	layer.full(index);
}
...
function copy(id){
	var index = layer.open({
		type: 2,
		title: '复制旧路面病害处治',
		content: 'diseaseTreatmentNumber_add.html?id='+id+"&flag=2",
	});
	layer.full(index);
}
...
function check(id){
	var index = layer.open({
		...
		title: '查看旧路面病害处治',
//		area: ['850px', '620px'],
		content: 'diseaseTreatmentNumber_add.html?id='+id+"&flag=0",
	});
	layer.full(index);
}
...
function del(id){
	layer.confirm('确认要删除吗？', 
		function(index) {
			...
			$.ajax({
				type: 'POST',
				url: '../../DiseaseTreatmentNumber/deleteDiseaseTreatmentNumber.action',
...
function exportInfo(){
	...
	param.countyId = countyId
	$.ajaxFileUpload({
		url : '../../DiseaseTreatmentNumber/export.action',
		data:param,
		secureuri : false,
		fileElementId : 'file',
