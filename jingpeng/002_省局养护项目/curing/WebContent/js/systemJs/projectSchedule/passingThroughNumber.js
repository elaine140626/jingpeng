...
function getList(treeIds,levels){
	...
	$("#table1").DataTable({
        ...
        "ajax" : {
            type: "post",
            url: "../../PassingThroughNumber/getPassingThroughNumberList.action",
            dataSrc: "data",
            data: function (d) {
            	param.cityId = cityId
...
function getSum(treeIds,levels){
	...
	$.ajax({
		type: 'POST',
		url: '../../PassingThroughNumber/getPassingThroughNumberSum.action',
		data:param,
		dataType: 'json',
		success: function(data){
			if (data[0] != null){
				$("#bridgeNameSum").html(data[0].bridgeNameSum);
				...
				$("#earthwork2Sum").html(data[0].earthwork2Sum);
	...
	if (selectedNodes != null && selectedNodes.length>0){
		if (selectedNodes[0].level != 3){
			...
			var index = layer.open({
				...
				title: '添加过路过户涵',
//				area: ['850px', '620px'],
				content: 'passingThroughNumber_add.html?ProjectId='+ProjectId,
			});
			layer.full(index);
		}
...
function update(id){
	var index = layer.open({
		...
		title: '修改过路过户涵',
//		area: ['850px', '620px'],
		content: 'passingThroughNumber_add.html?id='+id+"&flag=1",
	});
	layer.full(index);
}
...
function copy(id){
	var index = layer.open({
		type: 2,
		title: '复制过路过户涵',
		content: 'passingThroughNumber_add.html?id='+id+"&flag=2",
	});
	layer.full(index);
}
...
function check(id){
	var index = layer.open({
		...
		title: '查看过路过户涵',
//		area: ['850px', '620px'],
		content: 'passingThroughNumber_add.html?id='+id+"&flag=0",
	});
	layer.full(index);
}
...
function del(id){
	layer.confirm('确认要删除吗？', 
		function(index) {
			...
			$.ajax({
				type: 'POST',
				url: '../../PassingThroughNumber/deletePassingThroughNumber.action',
...
function exportInfo(){
	...
	param.countyId = countyId
	$.ajaxFileUpload({
		url : '../../PassingThroughNumber/export.action',
		data:param,
		secureuri : false,
		fileElementId : 'file',
