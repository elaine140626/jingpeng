...
function getList(treeIds,levels){
	...
	$("#table1").DataTable({
        ...
        "ajax" : {
            type: "post",
            url: "../../ProtectiveWallNumber/getProtectiveWallNumberList.action",
            dataSrc: "data",
            data: function (d) {
            	param.cityId = cityId
...
function getSum(treeIds,levels){	
	...
	$.ajax({
		type: 'POST',
		url: '../../ProtectiveWallNumber/getProtectiveWallNumberSum.action',
		data:param,
		dataType: 'json',
		success: function(data){
			if (data[0] != null){
				...
				$("#lacquerSum").html(data[0].lacquerSum);
			} else {
				$("#lengthSum").html("");
				$("#amountSum").html("");
				$("#c25ConcreteSum").html("");
	...
	if (selectedNodes != null && selectedNodes.length>0){
		if (selectedNodes[0].level != 3){
			...
			var index = layer.open({
				...
				title: '添加防护墙工程',
//				area: ['850px', '620px'],
				content: 'protectiveWallNumber_add.html?ProjectId='+ProjectId,
			});
			layer.full(index);
		}
...
function update(id){
	var index = layer.open({
		...
		title: '修改防护墙工程',
//		area: ['850px', '620px'],
		content: 'protectiveWallNumber_add.html?id='+id+"&flag=1",
	});
	layer.full(index);
}
...
function copy(id){
	var index = layer.open({
		type: 2,
		title: '复制防护墙工程',
		content: 'protectiveWallNumber_add.html?id='+id+"&flag=2",
	});
	layer.full(index);
}
...
function check(id){
	var index = layer.open({
		...
		title: '查看防护墙工程',
//		area: ['850px', '620px'],
		content: 'protectiveWallNumber_add.html?id='+id+"&flag=0",
	});
	layer.full(index);
}
...
function del(id){
	layer.confirm('确认要删除吗？', 
		function(index) {
			...
			$.ajax({
				type: 'POST',
				url: '../../ProtectiveWallNumber/deleteProtectiveWallNumber.action',
...
function exportInfo(){
	...
	param.countyId = countyId
	$.ajaxFileUpload({
		url : '../../ProtectiveWallNumber/export.action',
		data:param,
		secureuri : false,
		fileElementId : 'file',
