...
function getList(treeIds,levels){
	...
	$("#table1").DataTable({
        ...
        "ajax" : {
            type: "post",
            url: "../../BridgeRetainingWallNumber/getBridgeRetainingWallNumberList.action",
            dataSrc: "data",
            data: function (d) {
            	param.cityId = cityId
	...
	if (selectedNodes != null && selectedNodes.length>0){
		if (selectedNodes[0].level != 3){
			...
			var index = layer.open({
				...
				title: '添加桥梁挡墙工程',
//				area: ['850px', '620px'],
				content: 'bridgeRetainingWallNumber_add.html?ProjectId='+ProjectId,
			});
			layer.full(index);
		}
...
function getSum(treeIds,levels){	
	...
	$.ajax({
		type: 'POST',
		url: '../../BridgeRetainingWallNumber/getBridgeRetainingWallNumberSum.action',
		data:param,
		dataType: 'json',
		success: function(data){
			if (data[0] != null){
				...
				$("#backfillGravelSum").html(data[0].backfillGravelSum);
			} else {
				$("#wallLengthSum").html("");
				$("#stoneBasicsSum").html("");
...
function update(id){
	var index = layer.open({
		...
		title: '修改桥梁挡墙工程',
//		area: ['850px', '620px'],
		content: 'bridgeRetainingWallNumber_add.html?id='+id+"&flag=1",
	});
	layer.full(index);
}
...
function copy(id){
	var index = layer.open({
		type: 2,
		title: '复制桥梁挡墙工程',
		content: 'bridgeRetainingWallNumber_add.html?id='+id+"&flag=2",
	});
	layer.full(index);
}
...
function check(id){
	var index = layer.open({
		...
		title: '查看桥梁挡墙工程',
//		area: ['850px', '620px'],
		content: 'bridgeRetainingWallNumber_add.html?id='+id+"&flag=0",
	});
	layer.full(index);
}
...
function del(id){
	layer.confirm('确认要删除吗？', 
		function(index) {
			...
			$.ajax({
				type: 'POST',
				url: '../../BridgeRetainingWallNumber/deleteBridgeRetainingWallNumber.action',
...
function exportInfo(){
	...
	param.countyId = countyId
	$.ajaxFileUpload({
		url : '../../BridgeRetainingWallNumber/export.action',
		data:param,
		secureuri : false,
		fileElementId : 'file',
