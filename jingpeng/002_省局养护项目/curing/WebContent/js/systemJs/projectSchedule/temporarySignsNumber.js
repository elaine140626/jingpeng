...
function getList(treeIds,levels){
	...
	$("#table1").DataTable({
        ...
        "ajax" : {
            type: "post",
            url: "../../TemporarySignsNumber/getTemporarySignsNumberList.action",
            dataSrc: "data",
            data: function (d) {
            	param.cityId = cityId
...
function getSum(treeIds,levels){
	...
	$.ajax({
		type: 'POST',
		url: '../../TemporarySignsNumber/getTemporarySignsNumberSum.action',
		data:param,
		dataType: 'json',
	...
	if (selectedNodes != null && selectedNodes.length>0){
		if (selectedNodes[0].level != 3){
			...
			var index = layer.open({
				...
				title: '添加临时标志',
//				area: ['850px', '620px'],
				content: 'temporarySignsNumber_add.html?ProjectId='+ProjectId,
			});
			layer.full(index);
		}
...
function update(id){
	var index = layer.open({
		...
		title: '修改临时标志',
//		area: ['850px', '620px'],
		content: 'temporarySignsNumber_add.html?id='+id+"&flag=1",
	});
	layer.full(index);
}
...
function copy(id){
	var index = layer.open({
		type: 2,
		title: '复制临时标志',
		content: 'temporarySignsNumber_add.html?id='+id+"&flag=2",
	});
	layer.full(index);
}
...
function check(id){
	var index = layer.open({
		...
		title: '查看临时标志',
//		area: ['850px', '620px'],
		content: 'temporarySignsNumber_add.html?id='+id+"&flag=0",
	});
	layer.full(index);
}
...
function del(id){
	layer.confirm('确认要删除吗？', 
		function(index) {
			...
			$.ajax({
				type: 'POST',
				url: '../../TemporarySignsNumber/deleteTemporarySignsNumber.action',
...
function exportInfo(){
	...
	param.countyId = countyId
	$.ajaxFileUpload({
		url : '../../TemporarySignsNumber/export.action',
		data:param,
		secureuri : false,
		fileElementId : 'file',
