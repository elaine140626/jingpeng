	...
	if (selectedNodes != null && selectedNodes.length>0){
		if (selectedNodes[0].level != 3){
			...
			var index = layer.open({
				...
				title: '添加子目',
//				area: ['850px', '620px'],
				content: 'subheadNumberMoney_add.html?ProjectId='+ProjectId,
			});
			layer.full(index);
		}
...
// 编辑
function update(id, projectId){
	var index = layer.open({
		type: 2,
		title: '修改子目',
//		area: ['850px', '620px'],
		content: 'subheadNumberMoney_add.html?id='+id+'&ProjectId='+projectId+"&flag=1",
	});
	layer.full(index);
}

// 查看
function check(id, projectId){
	var index = layer.open({
		type: 2,
		title: '查看子目',
//		area: ['850px', '620px'],
		content: 'subheadNumberMoney_add.html?id='+id+'&ProjectId='+projectId+"&flag=0",
	});
	layer.full(index);
}
...
function del(id){
	layer.confirm('确认要删除吗？', 
		function(index) {
		var isHave = getSevenDailyById(id);
		if(isHave){
			// 操作失败
			layer.msg('已被7日报调用', {
				icon: 2,
			...
			$.ajax({
				type: 'POST',
				url: '../../SubheadNumberMoney/deleteSubheadNumberMoney.action',
...
function getSevenDailyById(id){
	var isNo = true;
	// 获取7日报
	$.ajax({
		type : "post",
		url : "../../SubheadNumberMoney/getSevenDailyById.action",
		async : false,
		data : {"id" : id},
		dataType : "json",
		success : function(data) {
