...
function getList(treeIds,levels){
	...
	$("#table1").DataTable({
        ...
        "ajax" : {
            type: "post",
            url: "../../CrossRoadbedNumber/getCrossRoadbedNumberList.action",
            dataSrc: "data",
            data: function (d) {
            	param.cityId = cityId
        ...
        "columns": [
        	...
        	{"data": "rebuildLength"},
        	{"data": "concrete"},
        	{"data": "modifiedAsphalt"},
        	{"data": "ordinaryAsphalt"},
        	{"data": "stickyLayer"},
        	{"data": "permeableLayer"},
        	{"data": "seal"},
        	{"data": "cementGravel20cm"},
        	{"data": "naturalGravel15cm"},
        	{"data": "naturalGravel20cm"},
        	{"data": "excavation"},
        	{"data": "fill"},
        	{"data": "remarks"}      	
        	],
...
function getSum(treeIds,levels){
	...
	$.ajax({
		type: 'POST',
		url: '../../CrossRoadbedNumber/getCrossRoadbedNumberSum.action',
		data:param,
		dataType: 'json',
		success: function(data){
			if (data[0] != null){
				$("#concreteSum").html(data[0].concreteSum);
				$("#modifiedAsphaltSum").html(data[0].modifiedAsphaltSum);
				$("#ordinaryAsphaltSum").html(data[0].ordinaryAsphaltSum);
				$("#stickyLayerSum").html(data[0].stickyLayerSum);
				$("#permeableLayerSum").html(data[0].permeableLayerSum);
				$("#sealSum").html(data[0].sealSum);
				$("#cementGravel20cmSum").html(data[0].cementGravel20cmSum);
				$("#naturalGravel15cmSum").html(data[0].naturalGravel15cmSum);
				$("#naturalGravel20cmSum").html(data[0].naturalGravel20cmSum);
				$("#excavationSum").html(data[0].excavationSum);
				$("#fillSum").html(data[0].fillSum);
			} else {
				$("#concreteSum").html("");
				$("#modifiedAsphaltSum").html("");
				$("#ordinaryAsphaltSum").html("");
				$("#stickyLayerSum").html("");
				$("#permeableLayerSum").html("");
				$("#sealSum").html("");
				$("#cementGravel20cmSum").html("");
				$("#naturalGravel15cmSum").html("");
				$("#naturalGravel20cmSum").html("");
				$("#excavationSum").html("");
				$("#fillSum").html("");
			}	
		}
	...
	if (selectedNodes != null && selectedNodes.length>0){
		if (selectedNodes[0].level != 3){
			...
			var index = layer.open({
				...
				title: '添加平面交叉路基路面',
//				area: ['850px', '620px'],
				content: 'crossRoadbedNumber_add.html?ProjectId='+ProjectId,
			});
			layer.full(index);
		}
...
function update(id){
	var index = layer.open({
		...
		title: '修改平面交叉路基路面',
//		area: ['850px', '620px'],
		content: 'crossRoadbedNumber_add.html?id='+id+'&flag=1',
	});
	layer.full(index);
}
...
function copy(id){
	var index = layer.open({
		type: 2,
		title: '复制平面交叉路基路面',
		content: 'crossRoadbedNumber_add.html?id='+id+'&flag=2',
	});
	layer.full(index);
}
...
function check(id){
	var index = layer.open({
		...
		title: '查看平面交叉路基路面',
//		area: ['850px', '620px'],
		content: 'crossRoadbedNumber_add.html?id='+id+'&flag=0',
	});
	layer.full(index);
}
...
function del(id){
	layer.confirm('确认要删除吗？', 
		function(index) {
			...
			$.ajax({
				type: 'POST',
				url: '../../CrossRoadbedNumber/deleteCrossRoadbedNumber.action',
...
/*	$.ajax({
	...
	$.ajaxFileUpload({
		url : '../../CrossRoadbedNumber/export.action',
		data:param,
		secureuri : false,
		fileElementId : 'file',
