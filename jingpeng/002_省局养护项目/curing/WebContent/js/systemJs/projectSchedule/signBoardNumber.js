...
function getList(treeIds,levels){
	...
	$("#table1").DataTable({
        ...
        "ajax" : {
            type: "post",
            url: "../../SignBoardNumber/getSignBoardNumberList.action",
            dataSrc: "data",
            data: function (d) {
            	param.cityId = cityId
        ...
        "columns": [
        	...
        	{"data": "nameSpecifications"},
        	{"data": "unit"},
        	{"data": "singleHangAmount"},
        	{"data": "singleHangWeight"},
        	{"data": "singleColumnAmount"},
        	{"data": "singleColumnWeight"},
        	{"data": "doubleColumnAmount"},
        	{"data": "doubleColumnWeight"},
        	{"data": "attachmentAmount"},
        	{"data": "attachmentWeight"},
        	{"data": "remarks"}      	
        	],
	...
	if (selectedNodes != null && selectedNodes.length>0){
		if (selectedNodes[0].level != 3){
			...
			var index = layer.open({
				...
				title: '添加标志牌工程',
//				area: ['850px', '620px'],
				content: 'signBoardNumber_add.html?ProjectId='+ProjectId,
			});
			layer.full(index);
		}
...
function update(id){
	var index = layer.open({
		...
		title: '修改标志牌工程',
//		area: ['850px', '620px'],
		content: 'signBoardNumber_add.html?id='+id+"&flag=1",
	});
	layer.full(index);
}
...
function copy(id){
	var index = layer.open({
		type: 2,
		title: '复制标志牌工程',
		content: 'signBoardNumber_add.html?id='+id+"&flag=2",
	});
	layer.full(index);
}
...
function check(id){
	var index = layer.open({
		...
		title: '查看标志牌工程',
//		area: ['850px', '620px'],
		content: 'signBoardNumber_add.html?id='+id+"&flag=0",
	});
	layer.full(index);
}
...
function del(id){
	layer.confirm('确认要删除吗？', 
		function(index) {
			...
			$.ajax({
				type: 'POST',
				url: '../../SignBoardNumber/deleteSignBoardNumber.action',
...
function exportInfo(){
	...
	param.countyId = countyId
	$.ajaxFileUpload({
		url : '../../SignBoardNumber/export.action',
		data:param,
		secureuri : false,
		fileElementId : 'file',
