...
function getList(treeIds,levels){
	...
	$("#table1").DataTable({
        ...
        "ajax" : {
            type: "post",
            url: "../../CulvertMaintenanceNumber/getCulvertMaintenanceNumberList.action",
            dataSrc: "data",
            data: function (d) {
            	param.cityId = cityId
	...
	if (selectedNodes != null && selectedNodes.length>0){
		if (selectedNodes[0].level != 3){
			...
			var index = layer.open({
				...
				title: '添加涵洞维修工程',
//				area: ['850px', '620px'],
				content: 'culvertMaintenanceNumber_add.html?ProjectId='+ProjectId,
			});
			layer.full(index);
		}
...
function getSum(treeIds,levels){
	...
	$.ajax({
		type: 'POST',
		url: '../../CulvertMaintenanceNumber/getCulvertMaintenanceNumberSum.action',
		data:param,
		dataType: 'json',
		success: function(data){
			if (data[0] != null){
				$("#siltSum").html(data[0].siltSum);
				$("#concreteSum").html(data[0].concreteSum);
				$("#bondedRebarsSum").html(data[0].bondedRebarsSum);
				$("#rubbleSum").html(data[0].rubbleSum);
			} else {
				$("#siltSum").html("");
				$("#concreteSum").html("");
				$("#bondedRebarsSum").html("");
				$("#rubbleSum").html("");
			}	
		}
	});			
...
function update(id){
	var index = layer.open({
		...
		title: '修改涵洞维修工程',
//		area: ['850px', '620px'],
		content: 'culvertMaintenanceNumber_add.html?id='+id+"&flag=1",
	});
	layer.full(index);
}
...
function copy(id){
	var index = layer.open({
		type: 2,
		title: '复制涵洞维修工程',
		content: 'culvertMaintenanceNumber_add.html?id='+id+"&flag=2",
	});
	layer.full(index);
}
...
function check(id){
	var index = layer.open({
		...
		title: '查看涵洞维修工程',
//		area: ['850px', '620px'],
		content: 'culvertMaintenanceNumber_add.html?id='+id+"&flag=0",
	});
	layer.full(index);
}
...
function del(id){
	layer.confirm('确认要删除吗？', 
		function(index) {
			...
			$.ajax({
				type: 'POST',
				url: '../../CulvertMaintenanceNumber/deleteCulvertMaintenanceNumber.action',
...
/*	$.ajax({
	...
	$.ajaxFileUpload({
		url : '../../CulvertMaintenanceNumber/export.action',
		data:param,
		secureuri : false,
		fileElementId : 'file',
