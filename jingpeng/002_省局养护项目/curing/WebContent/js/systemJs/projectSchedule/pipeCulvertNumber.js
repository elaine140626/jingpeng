...
function getList(treeIds,levels){
	...
	$("#table1").DataTable({
        ...
        "ajax" : {
            type: "post",
            url: "../../PipeCulvertNumber/getPipeCulvertNumberList.action",
            dataSrc: "data",
            data: function (d) {
            	param.cityId = cityId
        ...
        "columns": [
        	...
        	{"data": "pileNumber"},
        	{"data": "m15Body"},
        	{"data": "c25ConcreteWall"},
        	{"data": "c25ConcreteTube"},
        	{"data": "c25ConcreteCap"},
        	{"data": "gravelCushion"},
        	{"data": "rubbleHole"},
        	{"data": "rubbleWater"},
        	{"data": "m10RubbleBody"},
        	{"data": "c15StoneConcrete"},
        	{"data": "tubeRebar"},
        	{"data": "tubeConcrete"},
        	{"data": "backfillPellets"},
        	{"data": "excavation"},
        	{"data": "remarks"}     	
        	],
...
function getSum(treeIds,levels){
	...
	$.ajax({
		type: 'POST',
		url: '../../PipeCulvertNumber/getPipeCulvertNumberSum.action',
		data:param,
		dataType: 'json',
		success: function(data){
			if (data[0] != null){
				$("#m15BodySum").html(data[0].m15BodySum);
				$("#c25ConcreteWallSum").html(data[0].c25ConcreteWallSum);
				$("#c25ConcreteTubeSum").html(data[0].c25ConcreteTubeSum);
				$("#c25ConcreteCapSum").html(data[0].c25ConcreteCapSum);
				$("#gravelCushionSum").html(data[0].gravelCushionSum);
				$("#rubbleHoleSum").html(data[0].rubbleHoleSum);
				$("#rubbleWaterSum").html(data[0].rubbleWaterSum);
				$("#m10RubbleBodySum").html(data[0].m10RubbleBodySum);
				$("#c15StoneConcreteSum").html(data[0].c15StoneConcreteSum);
				$("#tubeRebarSum").html(data[0].tubeRebarSum);
				$("#tubeConcreteSum").html(data[0].tubeConcreteSum);
				$("#backfillPelletsSum").html(data[0].backfillPelletsSum);
				$("#excavationSum").html(data[0].excavationSum);
			} else {
				$("#m15BodySum").html("");
				$("#c25ConcreteWallSum").html("");
				$("#c25ConcreteTubeSum").html("");
				$("#c25ConcreteCapSum").html("");
				$("#gravelCushionSum").html("");
				$("#rubbleHoleSum").html("");
				$("#rubbleWaterSum").html("");
				$("#m10RubbleBodySum").html("");
				$("#c15StoneConcreteSum").html("");
				$("#tubeRebarSum").html("");
				$("#backfillPelletsSum").html("");
				$("#excavationSum").html("");
			}	
		}
	});			
	...
	if (selectedNodes != null && selectedNodes.length>0){
		if (selectedNodes[0].level != 3){
			...
			var index = layer.open({
				...
				title: '添加管涵工程',
//				area: ['850px', '620px'],
				content: 'pipeCulvertNumber_add.html?ProjectId='+ProjectId,
			});
			layer.full(index);
		}
...
function search(){	
	...
	if (selectedNodes[0] != undefined){
		levels = selectedNodes[0].level;
		if (levels !=0){
			treeIds = selectedNodes[0].id.substr(2,1);
		}		
	}	
	// datatable
	getList(treeIds,levels);
...
function update(id){
	var index = layer.open({
		...
		title: '修改管涵工程',
//		area: ['850px', '620px'],
		content: 'pipeCulvertNumber_add.html?id='+id+"&flag=1",
	});
	layer.full(index);
}
...
function check(id){
	var index = layer.open({
		...
		title: '查看管涵工程',
//		area: ['850px', '620px'],
		content: 'pipeCulvertNumber_add.html?id='+id+"&flag=0",
	});
	layer.full(index);
}
...
function del(id){
	layer.confirm('确认要删除吗？', 
		function(index) {
			...
			$.ajax({
				type: 'POST',
				url: '../../PipeCulvertNumber/deletePipeCulvertNumber.action',
