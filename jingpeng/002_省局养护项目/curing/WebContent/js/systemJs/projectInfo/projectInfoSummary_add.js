		...
		$.ajax({
			...
			success : function(data) {
				if(data != null && data.length > 0){
					...
					// 批文类型
					var testItems1 = data[0].approvalsType;
					if(testItems1 != null){
						var testItemList1 = testItems1.split(",");
						if(testItemList1 != null && testItemList1.length > 0){
							$("#approvalsType").select2().val(testItemList1).trigger("change");
						}
					}
				}
				...
				if(flag == 0){
					...
					$('#planTime').css('background-color','rgb(235, 235, 228)');
					// 隐藏保存按钮
					$("#saveButton").hide();
					...
					$('#planTime').css('background-color','#fff');
					// 显示保存按钮
					$("#saveButton").show();
				}
			}
