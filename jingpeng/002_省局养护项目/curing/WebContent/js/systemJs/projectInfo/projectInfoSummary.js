...
function update(id){
	var index = layer.open({
		type: 2,
		title: '修改工程基本信息',
		content: 'projectInfoSummary_add.html?id='+id+"&flag=1",
	});
	layer.full(index);
}
...
function copy(id){
	var index = layer.open({
		type: 2,
		title: '复制工程基本信息',
		content: 'projectInfoSummary_add.html?id='+id+"&flag=2",
	});
	layer.full(index);
}
...
function check(id){
	var index = layer.open({
		type: 2,
		title: '查看工程基本信息',
		content: 'projectInfoSummary_add.html?id='+id+"&flag=0",
	});
	layer.full(index);
}
...
function changePlanTime(){
	// 开始时间
	var startYear = $("#planTimeStr").val();
	// 结束时间
	var endYear = $("#planTimeEnd").val();
	
	if(startYear != "" && endYear != ""){
		if(startYear > endYear){
			layer.alert("计划年份开始年份大于结束年份!", {
				icon: 2,
				title: "提示"
			});
			return;
