...
$(function(){
	...
	if (id != null && id != ""){
		...
		$.ajax({
			type : "post",
			url : "../../ProjectInfoSummary/selectFileList1.action",
			data : {"id" : id},
			dataType : "json",
...
function delOrDownImage(count){
	...
	if(count == 1){
		url =  "../../ProjectInfoSummary/delUploadfile.action";
	}else{
		window.location.href = "../../ProjectInfoSummary/downUploadfile.action?imageNamePrams=" + imageNameStringList+"&imagePrefix="+imagePrefix;
	}
	// 删除所选图片信息
	$.ajax({
		type : "post",
